export AlgClf,
  AlgClfElem, clifford_algebra, dim_qf, even_coeffs, odd_coeffs, even_part, odd_part, coeffs
...
@doc raw"""
    centroid(C::AlgClf) -> TODO

Return the centroid of $C$ as a 2-dimensional étale algebra, together with an
embedding into $C$. The centroid is the centraliser of the even Clifford algebra.
"""
function centroid(C::AlgClf)
  #TODO
  return 0
end

@doc raw"""
    center(C::AlgClf) -> TODO

Return the center of $C$ as a 2-dimensional étale algebra, together with an
embedding into $C$. The center is equal to the base ring of $C$, if $dim(C)$
is even and equal to the centroid otherwise.
"""
function center(C::AlgClf)
  #TODO
...
function _mul_baseelt_with_gen(x::BitVector, i::Int, gram::MatElem{T}) where {T<:RingElem}
  R = base_ring(gram)
  j = _get_last(x)
